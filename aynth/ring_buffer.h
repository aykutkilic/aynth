...
inline void single_write_ring_buffer( ring_buffer * rb, int32_t index, double value ) {
	rb->buffer[ (rb->cursor + index) % rb->size ] = value;
}

inline double single_read_ring_buffer( ring_buffer * rb, int32_t index ) {
...
void linear_read( ring_buffer * rb, int32_t offset, uint32_t count, void * out );
